...
6. **공유 기능에는 앱으로 들어오는 링크를 무조건 같이 보낸다.** 이미지만 보내는 공유는 만들지 않는다.
   기본 버튼은 `Share.share({ message })`로 문구와 링크를 보내고(두 OS 동일, 링크가 눌린다),
   이미지 파일이 필요할 때만 `Sharing.shareAsync`를 보조 버튼으로 둔다.
   `Share.share`의 `url`은 iOS 전용이라 안드로이드에서는 조용히 빠진다 — 이미지 첨부에 쓰지 않는다.
